...
class Node():
    ...
    def get_points_loose(self, domain, bondx = "OPEN"): 
        ...
        if bondx == "PBC":
            domain_vcat = np.concatenate((domain[-1].reshape((1, nx)), domain, domain[0].reshape((1, nx))))
            domain_cat = np.hstack((domain_vcat[:, -1].reshape((ny + 2, 1)), domain_vcat, domain_vcat[:, 0].reshape((ny + 2, 1))))
            smallx = self.x0
    ...
    def get_corners_points(self, domain, ind):
        if ind == 0:
            return domain[self.x0:self.x0 + int(self.width/2), self.y0:self.y0 + int(self.height/2)]
        elif ind == 1:
            return domain[self.x0 + int(self.width/2): self.x0 + self.width, self.y0: self.y0 + int(self.height/2)]
        elif ind == 2:
            return domain[self.x0:self.x0 + int(self.width/2), self.y0 + int(self.height/2):self.y0 + self.height]
        elif ind == 3:
            return domain[self.x0 + int(self.width/2): self.x0 + self.width, self.y0 + int(self.height/2):self.y0 + self.height]
    ...
    def recursive_subdivide(self, threshold, minCellSize, domain, bondx = "OPEN"):
        if self.get_grad(domain, bondx) <= threshold: #no refinement if the gradient is too small
            return
...
class QTree():
    ...
    def subdivide(self):
        self.root.recursive_subdivide(self.threshold, self.minCellSize, self.domain, self.bondx)
